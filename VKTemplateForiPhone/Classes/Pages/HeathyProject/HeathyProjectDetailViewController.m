//
//  HeathyProjectDetailViewController.m
//  VKTemplateForiPhone
//
...
@interface HeathyProjectDetailViewController ()<UIWebViewDelegate>{
     BOOL isLoadingFinished;
}

@end

@implementation HeathyProjectDetailViewController
@synthesize planContent;
...
- (void)viewDidLoad {
    ...
    //第一次加载先隐藏webview
    [wView setHidden:YES];
    [wView loadHTMLString:OptimizeHtmlString(self.planContent) baseURL:nil];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    //获取实际要显示的html
    NSString *html = [self htmlAdjustWithPageWidth:widthOfBody html:OptimizeHtmlString(self.planContent) webView:webView];
    //设置为已经加载完成
