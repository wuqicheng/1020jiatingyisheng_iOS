...
@synthesize webAppLink,wView,htmlContent;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!htmlContent) {
        NSURL *url = [NSURL URLWithString:webAppLink];
        ...
        //第一次加载先隐藏webview
        [wView setHidden:YES];
        [wView loadHTMLString:OptimizeHtmlString(htmlContent) baseURL:nil];
        wView.delegate = self;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    //获取实际要显示的html
    NSString *html = [self htmlAdjustWithPageWidth:widthOfBody html:OptimizeHtmlString(htmlContent) webView:webView];
    //设置为已经加载完成
    ...
    wView.frame = CGRectMake(wView.frame.origin.x,wView.frame.origin.y, [UIScreen mainScreen].bounds.size.width, Screen_Height-64);
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    NSLog(@"-- error when loading web page:%@ --",error);
    [SVProgressHUD dismiss];
