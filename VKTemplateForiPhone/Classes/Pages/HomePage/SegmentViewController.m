...
#define Default_BottomLineHeight    0
...
- (void)addButtonInScrollHeader:(NSArray *)titleArray
{
    ...
    for (NSInteger index = 0; index < titleArray.count; index++) {
        ...
        if (index == _selectIndex0) {
            segmentBtn.selected = YES;
            self.selectIndex = segmentBtn.tag;
            if ((self.selectIndex-HEADBTN_TAG)>=2&&(self.selectIndex-HEADBTN_TAG)<=(titleArray.count-3)) {
                self.headerView.contentOffset = CGPointMake((self.selectIndex-HEADBTN_TAG-1)*MainScreenWidth/4, 0);
            } if ((self.selectIndex-HEADBTN_TAG)>=3&&(self.selectIndex-HEADBTN_TAG)<(titleArray.count-1)){
                 self.headerView.contentOffset = CGPointMake((self.selectIndex-HEADBTN_TAG-2)*MainScreenWidth/4, 0);
                
            }if ((self.selectIndex-HEADBTN_TAG)==(titleArray.count-1)) {
                self.headerView.contentOffset = CGPointMake((MainScreenWidth/4) * (titleArray.count-4), 0);
            }
//            else{
//                 self.headerView.contentOffset = CGPointMake((titleArray.count)*Default_ButtonHeight, 0);
//            }
        }else{
            NSLog(@"nihaonihaonihao");
        }
    }
...
- (void)btnClick:(UIButton *)button
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SelectVC" object:button userInfo:nil];
}
...
- (void)didSelectSegmentIndex:(NSInteger)index
{
    ...
    [_delegate didSelectSegmentIndex:index];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (scrollView == _mainScrollView) {
        if (self.selectIndex == 10003) {
            float xx = scrollView.contentOffset.x * (3) * (_buttonWidth / MainScreenWidth) - _buttonWidth;
            [_headerView scrollRectToVisible:CGRectMake(xx, 0, MainScreenWidth, _headerView.frame.size.height) animated:YES];
            NSInteger currentIndex = scrollView.contentOffset.x *  (3) / MainScreenWidth;
            [self didSelectSegmentIndex:currentIndex + HEADBTN_TAG];
        }else{
        float xx = scrollView.contentOffset.x * (self.selectIndex-10000+1) * (_buttonWidth / MainScreenWidth) - _buttonWidth;
        [_headerView scrollRectToVisible:CGRectMake(xx, 0, MainScreenWidth, _headerView.frame.size.height) animated:YES];
        NSInteger currentIndex = scrollView.contentOffset.x *  (self.selectIndex-10000+1) / MainScreenWidth;
        [self didSelectSegmentIndex:currentIndex + HEADBTN_TAG];
        }
    }
}
