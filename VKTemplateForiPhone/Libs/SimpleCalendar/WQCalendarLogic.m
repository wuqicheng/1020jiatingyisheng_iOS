...
@property(nonatomic,strong) NSDate *getTheDate;
...
- (void)calculateDaysInPreviousMonthWithDate:(NSDate *)date
{
    ...
    NSUInteger partialDaysCount = weeklyOrdinality -2 ;
    
    NSDateComponents *components = [dayInThePreviousMonth YMDComponents];
    
    self.daysInPreviousMonth = [NSMutableArray arrayWithCapacity:100];
...
- (void)calculateDaysInFollowingMonthWithDate:(NSDate *)date
{
    ...
    if (weeklyOrdinality == 8) return ;
    
    NSUInteger partialDaysCount = 8 - weeklyOrdinality;
    NSDateComponents *components = [[date dayInTheFollowingMonth] YMDComponents];
    
    self.daysInFollowingMonth = [NSMutableArray arrayWithCapacity:100];
...
//{
////    tileView.label.titleLabel = [NSString stringWithFormat:@"%lu", (unsigned long)calendarDay.day];
//    [tileView.label setTitle:[NSString stringWithFormat:@"%lu", (unsigned long)calendarDay.day] forState:UIControlStateNormal];
...
//        [tileView.label setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
//    }
//    
...
////        tileView.label.textColor = [UIColor blackColor];
//        [tileView.label setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
//    }
//}
...
- (void)gridView:(WQCalendarGridView *)gridView didSelectAtRow:(NSUInteger)row column:(NSUInteger)column
{
    ...
    self.selectedDate = [self.selectedCalendarDay date];
     _getTheDate=[[NSDate alloc] initWithTimeInterval:36000 sinceDate:self.selectedDate];
    
    NSDateFormatter *date = [[NSDateFormatter alloc] init];
    [date setDateFormat:@"yyyy-MM-dd"];
    NSString *dateStr = [date stringFromDate:_getTheDate];
...
- (WQCalendarTileView *)scrollCalendarView:(WQScrollCalendarView *)scrollCalendarView tileViewForRow:(NSUInteger)row column:(NSUInteger)column
{
    ...
    if (calendarDay.month != self.selectedCalendarDay.month) {
        ...
        tileView.label.backgroundColor = [UIColor redColor];
